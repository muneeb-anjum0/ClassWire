...
The `v1` directory contains the reviewed production artifact generated by the
ClassWire Kaggle pipeline. Its metadata identifies model version
`tinybert-nlu-v4`, and the runtime verifies the ONNX checksum before loading it.
...
4. Install the lightweight runtime dependencies through `requirements.txt`.
